...
module.exports.profile_post = async (req, res) => {
  const { newName, newEmail, newPhone, newAddress, oldPassword, newPassword } =
    req.body;
  if (res.locals.user) {
    try {
  ...
  if (res.locals.user) {
    if (res.locals.user.role === "admin") {
      try {
        const {
          orderCodeSearch,
          orderCustomerCodeSearch,
          orderCustomerNameSearch,
          orderStatusSearch,
          orderFromDaySearch,
          orderToDaySearch,
        } = req.body;
        ...
        if (
          ...
          orderList = await Order.find({ orderCode: orderCodeSearch }).sort({
            createdAt: -1,
          });
          ...
          if (orderStatusSearch !== "Tất cả") {
            if (orderFromDaySearch !== "" && orderToDaySearch !== "") {
              console.log(
                "search by customer code, order status, from date and to date"
              );
              ...
              orderList = await Order.find({
                userCode: orderCustomerCodeSearch,
          ...
          if (orderStatusSearch !== "Tất cả") {
            if (orderFromDaySearch !== "" && orderToDaySearch !== "") {
              console.log(
                "search by customer name, order status, from date and to date"
              );
              var fromDate = new Date(orderFromDaySearch);
              fromDate = fromDate.setHours(fromDate.getHours() - 7);
              fromDate = new Date(fromDate);
              ...
              toDate = new Date(toDate);
              orderList = await Order.find({
                fullname: orderCustomerNameSearch,
                status: orderStatusSearch,
                createdAt: { $gte: fromDate, $lte: toDate },
              }).sort({ createdAt: -1 });
            } else if (orderFromDaySearch !== "") {
          ...
          if (orderFromDaySearch !== "" && orderToDaySearch !== "") {
            ...
            orderList = await Order.find({ status: orderStatusSearch }).sort({
              createdAt: -1,
            });
          }
          ...
          fromDate = fromDate.setHours(fromDate.getHours() - 7);
          fromDate = new Date(fromDate);
          orderList = await Order.find({ createdAt: { $gte: fromDate } }).sort({
            createdAt: -1,
          });
        } else if (orderToDaySearch !== "") {
          ...
          toDate = toDate.setHours(toDate.getHours() - 7 + 24);
          toDate = new Date(toDate);
          orderList = await Order.find({ createdAt: { $lte: toDate } }).sort({
            createdAt: -1,
          });
        }
    ...
    if (res.locals.user.role === "admin") {
      try {
        ...
        if (voucher !== "") {
          ...
          if (voucherInfo) {
            if (
              voucherInfo.codeList.find(
                (element) => element.code === voucher && element.isUsed === true
              )
            ) {
    ...
    if (res.locals.user.role === "admin") {
      try {
        ...
        if (voucher) {
          console.log(voucher);
          if (
            voucher.codeList.find(
              (element) =>
                element.code === voucherCode && element.isUsed === true
            )
          ) {
  ...
  if (res.locals.user) {
    if (res.locals.user.role === "admin") {
      try {
        ...
        await Order.findOneAndDelete({ orderCode: orderCode }).then(
          (result) => {
            if (result) {
...
module.exports.history_get = async (req, res) => {
  if (res.locals.user) {
    ...
    let filtersAnd = [{ authorId: req?.userId }];
    ...
    const historyList = await History.find(filters)
      .sort({ createdAt: -1 })
    ...
    if (res.locals.user.role === "admin") {
      try {
        ...
        if (
          ...
          voucherList = await Voucher.find({ voucherId: voucherIdSearch }).sort(
            { createdAt: -1 }
          );
        } else if (voucherCodeSearch !== "") {
          console.log("search by voucher code");
          voucherList = await Voucher.find({
            "codeList.code": voucherCodeSearch,
          }).sort({ createdAt: -1 });
          ...
          if (voucherExpSearch !== "") {
            ...
            voucherList = await Voucher.find({ type: voucherTypeSearch }).sort({
              createdAt: -1,
            });
          }
          ...
          var expDate = new Date(voucherExpSearch);
          expDate.setHours(0, 0, 0);
          voucherList = await Voucher.find({ exp: { $gte: expDate } }).sort({
            createdAt: -1,
          });
          var i = 0;
          expDate.setHours(23, 59, 59);
    ...
    if (res.locals.user.role === "admin") {
      try {
        ...
        if (
          ...
          newVoucherType === "" ||
          newVoucherValue === "" ||
          (newVoucherValue === "Phần trăm" &&
            (newVoucherMaxValue || newVoucherMaxValue === "")) ||
          newVoucherQuantity === "" ||
          newVoucherExp === ""
        ) {
